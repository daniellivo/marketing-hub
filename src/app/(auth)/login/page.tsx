...
function LoginForm() {
  ...
  useEffect(() => {
    ...
    if (error === 'unauthorized' && errorEmail) {
      toast.error(`El email ${errorEmail} no está autorizado para acceder a la plataforma.`)
    } else if (error === 'auth-failed') {
      toast.error('Error al autenticar con Google. Por favor intenta de nuevo.')
    } else if (error === 'no-email') {
      toast.error('No se pudo obtener tu email de Google.')
    }
  }, [error, errorEmail])
  ...
  async function handleGoogleLogin() {
    try {
      ...
      const { error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: `${process.env.NEXT_PUBLIC_SITE_URL || 'https://eoqlhmzrodxfpvviugll.supabase.co'}/auth/callback?redirectTo=${redirectTo}`,
          queryParams: {
            access_type: 'offline',
            prompt: 'consent',
          }
